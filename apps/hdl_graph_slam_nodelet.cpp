...
public:
  ...
  virtual void onInit() {
    ...
    seq_ = private_nh.param<std::string>("Sequence", "seq");
    std::cout<<"seq: "<<seq_.c_str()<<std::endl;
    use_test_ = private_nh.param<bool>("use_prev_test_", false);
...
private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg, const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    pcl::Filter<PointT>::Ptr keyframe_downsample_filter;
    double res_ = 0.5;
      auto keyframe_voxelgrid = new pcl::VoxelGrid<PointT>();
  ...
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    cout<<"begin save trajectory"<<endl;

    string trajfile("/home/wjk/hdl_graph_slam_catkin_ws/data/");
    trajfile += seq_.c_str();
    if(use_test_)
        trajfile += "_usetest.txt";
    else
        trajfile += ".txt";
    std::cout<<"trajfile: "<<trajfile.c_str()<<std::endl;
      ofstream traj(trajfile.c_str());
      if (!traj.is_open())
      {
      ...
      Eigen::Quaterniond q;
      Eigen::Matrix3d  R;
      Eigen::Isometry3d keyframe_T;
      Eigen::Matrix4d keyframe_M;
      for(int i = 0; i < keyframes.size(); i++) {
          keyframe_T = keyframes[i]->node->estimate();
          keyframe_M = keyframe_T.matrix();
          R = keyframe_M.block<3,3>(0,0);
          q = R;
          traj<<keyframes[i]->stamp<<" "<<keyframe_M(0,3)<<" "<<keyframe_M(1,3)<<" "<<keyframe_M(2,3)<<" "<<q.x()<<" "<<q.y()<<" "<<q.z()<<" "<<q.w()<<endl;
    }
    traj.close();
      cout<<"total distance is "<<endl;
     cout<<keyframe_updater->get_accum_distance()<<endl;
