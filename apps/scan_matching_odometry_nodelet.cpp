...
private:
  ...
  void initialize_params() {
    ...
    Thre_ = pnh.param<double>("Thre_", 1.0);
    Time_ = pnh.param<double>("Time_", 1.0);
    std::cout<<"Thre_ and Time_: "<<Thre_<<" "<<Time_<<std::endl;
    ...
    double downsample_resolution = pnh.param<double>("downsample_resolution", 0.25);
  ...
  Eigen::Matrix4f matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    if(!keyframe) {
      ...
      prev_trans0_.setIdentity();
      keyframe_pose.setIdentity();
      keyframe_stamp = stamp;
//
      keyframe = downsample(cloud);
      registration->setInputTarget(keyframe);
      Accumulated_trans_.setIdentity();
      FA.setInitTargetCloud(cloud);
      last_frame_ = cloud;
    ...
    FA.setSourceCloud(cloud);
    clock_t end_s = clock();
    std::cout<<"setSourceCloud "<<(double)(end_s-start_s)/CLOCKS_PER_SEC<<std::endl;
    FA.Thre_ = Thre_;
    FA.Time_ = Time_;
    clock_t start_ = clock();
    FA.runFeatureAssociation(prev_trans0_);
    clock_t end_ = clock();
    std::cout<<"odometry running speed "<<(double)(end_-start_)/CLOCKS_PER_SEC<<std::endl;
    last_frame_ = cloud;
    FA.setTargetCloud(last_frame_);
    ...
    trans0 = FA.getFinalTransformation();
    //std::cout<<"trans0: "<<std::endl<<trans0<<std::endl;
    Eigen::Matrix4f trans1 = Accumulated_trans_ * trans0;
    Accumulated_trans_ = trans1;
    Eigen::Matrix4f trans = Accumulated_trans_;
    Eigen::Matrix4f odom = keyframe_pose * trans;
    //std::cout<<"trans: "<<std::endl<<trans<<std::endl;
    //std::cout<<"prev_trans: "<<std::endl<<prev_trans<<std::endl;
    ...
    if(transform_thresholding) {
      ...
      //if uniform motion is not satisfied, will not accept this result.
    ...
    //
    std::cout<<"delta_trans and time: "<<delta_trans<<" "<<delta_angle<<" "<<delta_time<<std::endl;
    ...
    if(delta_trans > keyframe_delta_trans || delta_angle > keyframe_delta_angle || delta_time > keyframe_delta_time) {
        ...
        clock_t end = clock();
        std::cout<<"GICP running speed "<<(double)(end-start)/CLOCKS_PER_SEC<<std::endl;
        ...
        registration->setInputTarget(keyframe);
        Accumulated_trans_.setIdentity();
        odom = keyframe_pose * registration->getFinalTransformation();
        keyframe_pose = odom;
        keyframe_stamp = stamp;
        prev_time = stamp;
        prev_trans.setIdentity();
...
private:
  ...
  pcl::PointCloud<PointT>::ConstPtr last_frame_;  // keyframe point cloud
  Eigen::Matrix4f Accumulated_trans_;
  Eigen::Matrix4f prev_trans0_; //used for save the scan-to-scan matching
  pcl::PointCloud<PointT> Accumulated_frames_;
  int Accumulated_no_;
  int frames_num_;
