...
private:
  ...
  void cloud_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    Eigen::Matrix4f pose = matching(cloud_msg->header.stamp, cloud);
      clock_t end = clock();
      std::cout<<"matching time cost: "<<(double)(end-start)/CLOCKS_PER_SEC<<std::endl;
...
//
    ...
    clock_t t1 = clock();
    std::cout<<"align time: "<<(double)(t1-t0)/CLOCKS_PER_SEC<<std::endl;
    publish_scan_matching_status(stamp, cloud->header.frame_id, aligned, msf_source, msf_delta);
      clock_t t2 = clock();
      std::cout<<"publish_scan_matching_status time: "<<(double)(t2-t1)/CLOCKS_PER_SEC<<std::endl;
